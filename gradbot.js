...
function MarkerView(part) {
    ...
    this.draw = function(canvas, context) {
        ...
        if(this.part.penDrawing && this.loc && this.prevLoc && simState.running) {
            var canvas = document.getElementById("simbg"); // zz if changed to simfg, line disappears
 ...
 */
function drawSim() {
    console.log("Draw Sim Calledddd");
    var canvas = document.getElementById('simfg');
    var context = canvas.getContext("2d");

    // !!!!!!!!!! Sam Elfrink Addition zzz !!!!!!!!!!!!!!!!
    //var drawData = context.getImageData(0, 0, canvas.width, canvas.height);
    
    //console.log(simView.opponentThread);
    //console.log(opponentView);
    /*
    if(opponentView) {
        console.log("wipe them out");
        opponent = null;
    ...
    //console.log(simState.worldObjects);
...
function simulationReset(event) {
    ...
    graphPaperFill("simbg"); // zzz if this is removed, the draw lines will remain
