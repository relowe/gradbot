...
function VectorView(x, y, heading, scale, points, parent) {
    // set up the fields
    Positionable.call(this, x, y, heading);
    ...
    this.draw = function(canvas, context) {
        ...
        for(var i in points) {
            ...
            // update the position based on the parent view's position
            if (this.parent) {
                x += this.parent.x;
                y += this.parent.y;
            }
...
function ChassisView(part) {
    ...
    // create my vector view with the parent view as an argument
    this.view = new VectorView(0, 0, part.heading, 1.0, points);
...
function simAddLight(event) {
    ...
    var light = Bodies.circle(canvas.width/2, canvas.height/2, 5, {isStatic: false, render: {fillStyle: 'transparent', strokeStyle: 'black', lineWidth: 2 } });
    Composite.add(engine.world, [light]);
...
function simAddWall(event) {
    ...
    var wall = Bodies.rectangle(canvas.width/2, canvas.height/2, 18, 35, {isStatic: false, friction: 10000, frictionAir: 1, mass: 10000, render: {fillStyle: 'blue', strokeStyle: 'lightblue', lineWidth: 4 } });
...
function simAddBox(event) {
    var canvas = document.getElementById("engine");
    var box = Bodies.rectangle(canvas.width/2, canvas.height/2, 30, 30, {isStatic: false, render: {fillStyle: 'lightblue', strokeStyle: 'blue', lineWidth: 4 } });
...
function simAddHitbox(event) {
    var canvas = document.getElementById("engine");
    
    // add bodies
    var chassisPart = Bodies.rectangle(100, 100, 40, 24, { render: {fillStyle: 'transparent', strokeStyle: 'red', lineWidth: 1 }}),
        motorPart = Bodies.rectangle(86, 100, 80 / 6,  34, { render: chassisPart.render });

    var Hitbox = Body.create({
        isStatic: true, parts: [chassisPart, motorPart],
    });

    // update Hitbox position based on robot position
    Hitbox.position.x = robot.x;
    Hitbox.position.y = robot.y;

    Composite.add(engine.world, [Hitbox]);
    drawSim();
    console.log(Hitbox)
}
...
// Create the Hitbox object
function simAddOppHitbox(event) {
    var canvas = document.getElementById("engine");
    
    // add bodies
        oppChassisPart = Bodies.rectangle(700, 500, 40, 24, { render: {fillStyle: 'transparent', strokeStyle: 'red', lineWidth: 1 }}),
        oppMotorPart = Bodies.rectangle(714, 500, 80 / 6,  34, { render: oppChassisPart.render });

        var OppHitbox = Body.create({
            isStatic: true, parts: [oppChassisPart, oppMotorPart],
        });

    // update Hitbox position based on robot position
    OppHitbox.position.x = opponent.x;
    OppHitbox.position.y = opponent.y;

    Composite.add(engine.world, [OppHitbox]);
    drawSim();
    console.log(OppHitbox)
...
function gradbotInit() {
    ...
    document.getElementById('simAddHitbox').onclick = simAddHitbox;
    document.getElementById('simAddOppHitbox').onclick = simAddOppHitbox;
...
function simulationStop() {
    ...
    // remove the robot's ability to teleport on resume ^_^
...
//Runs the render, runner, and engine
Render.run(render);
